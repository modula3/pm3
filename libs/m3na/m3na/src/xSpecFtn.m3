MODULE xSpecFtn EXPORTS xReal64;
...
BEGIN
  IF n<=max THEN
    RETURN cache[n];
  ELSE
    RETURN exp(ln_gamma(FLOAT(n,REAL64)+One));
  END;
END factorial;
(*--------------------*)
CONST max_factln = 70; 
VAR   factln_cache:ARRAY [2..max_factln] OF REAL64;
...
BEGIN
  ...
  ELSIF n>max_factln THEN
    RETURN ln_gamma(FLOAT(n,REAL64)+One);
  END;

  (*---check the cache---*)
  tmp:=factln_cache[n];
  IF tmp=Zero THEN
    tmp:=ln_gamma(FLOAT(n,REAL64)+One);
    factln_cache[n]:=tmp;
  END;
  RETURN tmp;  
END ln_factorial;
...
(*returns ln(gamma(z))*)
TYPE coefs=ARRAY [0..N] OF REAL64;
CONST
  N=6;
  gam=5.0d0;
...
END gamma_q;
(*-------------------*)
PROCEDURE gamser(a,x:REAL64):REAL64 RAISES {Error}=
...
END gamser;
(*-------------------*)
PROCEDURE gamcf(a,x:REAL64):REAL64 RAISES {Error}=
(*helper for gamma_p and gamma_q*)
...
END beta;
(*--------------------*)
PROCEDURE betacf(a,b,x:REAL64):REAL64 RAISES {Error}=
...
CONST
  ...
  MaxIter=90;
VAR
  f,D,C,m,am,a2m,aj,delta:REAL64;
BEGIN
  (*---initialize at j=2---*)
  aj:=-(a+b)*x/(a+One);
  D:=One+aj;
