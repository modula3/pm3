...
(* Last modified on Wed Aug 17 15:49:39 PDT 1994 by kalsow     *)
(*      modified on Tue Jun  1 13:09:34 PDT 1993 by mcjones    *)
(*      modified on Thu Nov 12 11:48:08 PST 1992 by muller     *)
(*      modified on Fri Jun 29 10:50:46 1990 by piet@cs.ruu.nl *)
...
CONST
  ...
  FDEFER =     8_000040;        (* defer for next gc pass *)
  FCREAT =     8_000400;        (* create if nonexistant *)
  FTRUNC  =    8_001000;        (* truncate to zero length *)
  FEXCL =      8_002000;        (* error if already created *)
  FSYNCRON =   8_100000;        (* write file synchronously *)
  FSYNCIO  =   8_100000;        (* write file synchronously *)
  FNBLOCK =    8_200000;        (* POSIX no delay *)
...
CONST (* l_type values for Record Locking With fcntl() *)
  F_RDLCK = 1;
  F_WRLCK = 2;
  F_UNLCK = 3;
...
CONST
  ...
  F_TEST  = 3;	(* Test a region for a previous lock *)

<*EXTERNAL*> PROCEDURE lockf (fd, request: int; size: long): int;
...
(* this is a temptative declaration of the types of the arguments. It should
   probably be done in another way, so it is commented. We really need
...
TYPE
  struct_termio = RECORD
	...
	c_cc: ARRAY [1..NCC] OF u_char;	(* control chars *)
	END;
...
CONST
  ...
  IOC_VOID  = Shift (1, 31);       (* no parameters *)
...
CONST (* the tty i/o controls *)
  ...
  FIOSNBIO = FIONBIO;
  FIOSSAIOSTAT = Or (Or (W, INT), Or (FC, 122)); (* Set/clear async i/o    *)
  FIOGSAIOSTAT = Or (Or (W, INT), Or (FC, 121)); (* Get async i/o    *)
  FIOSSAIOOWN = Or (Or (W, INT), Or (FC, 120));	(* set sys async i/o owner *)
  FIOGSAIOOWN = Or (Or (W, INT), Or (FC, 119));	(* get sys async i/o owner *)
  FIOGNBIO = Or (Or (W, INT), Or (FC, 122));	(* arg is set:
				to 1, if non-blocking, to 0, otherwise *)
...
<*EXTERNAL*> PROCEDURE mkfifo (path: char_star; mode: int): int;
...
CONST (* flags *)
  ...
  O_NOCTTY =	8_400000;  	(* Don't assign controlling tty *)
  O_SYNC =      FSYNCRON;       (* syncronous write *)
  O_SYNCIO =    FSYNCRON;       (* syncronous write *)
...
CONST (* howto *)
  ...
  RB_NOSYNC =  16_4;		(* dont sync before reboot *)
...
<*EXTERNAL*> PROCEDURE truncate (path: char_star; length: u_long): int;

<*EXTERNAL*> PROCEDURE ftruncate (fd, length: u_long): int;
