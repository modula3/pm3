...
FROM RTHeapRep
IMPORT Header, RefHeader, AllocTraced, AllocUntraced, AllocPool,
       newPool, newTransient;
...
PROCEDURE NewTraced(tc: Typecode): REFANY
  ...
  VAR
    ...
    res: REFANY;
  BEGIN
    IF def.traced = ORD (RK.Traced) THEN
      res := GetTraced(def, newPool);
    ELSE
      <* ASSERT def.traced = ORD (RK.Transient) *>
      res := GetTraced(def, newTransient);
    END;
...
PROCEDURE NewTracedArray(tc: Typecode; READONLY s: Shape): REFANY
  ...
  BEGIN
    IF def.traced = ORD (RK.Traced) THEN
      res := GetOpenArray(def, s, newPool);
    ELSE
      <* ASSERT def.traced = ORD (RK.Transient) *>
      res := GetOpenArray(def, s, newTransient);
    END;
    IF (res = NIL) THEN RAISE OutOfMemory; END;
    RETURN res;
  END NewTracedArray;
...
PROCEDURE Allocate (defn: ADDRESS): REFANY =
  VAR res := GetTraced(defn, newPool);
  BEGIN
...
PROCEDURE AllocateTransient (defn: ADDRESS): REFANY =
  VAR res := GetTraced(defn, newTransient);
  BEGIN
    IF (res = NIL) THEN  RAISE RTE.E (RTE.T.OutOfMemory);  END;
    RETURN res;
...
PROCEDURE AllocateOpenArray (defn: ADDRESS; READONLY s: Shape): REFANY =
  VAR res := GetOpenArray(defn, s, newPool);
  BEGIN
    IF (res = NIL) THEN  RAISE RTE.E (RTE.T.OutOfMemory);  END;
    RETURN res;
...
PROCEDURE AllocateTransientOpenArray (defn: ADDRESS; READONLY s: Shape):
  REFANY =
  VAR res := GetOpenArray(defn, s, newTransient);
  BEGIN
    IF (res = NIL) THEN  RAISE RTE.E (RTE.T.OutOfMemory);  END;
    RETURN res;
...
PROCEDURE GetTraced (defn: ADDRESS; VAR pool: AllocPool): REFANY =
  ...
  BEGIN
    IF (tc = 0)
      OR (def.traced = ORD (RK.Untraced))
      OR (def.kind = ORD (TK.Array)) THEN
      <*NOWARN*> EVAL VAL (-1, CARDINAL); (* force a range fault *)
    END;

    RTOS.LockHeap();

      res := AllocTraced(def.dataSize, def.dataAlignment, pool);
...
PROCEDURE GetOpenArray (defn: ADDRESS; READONLY s: Shape;
                        VAR pool: AllocPool): REFANY =
  ...
  BEGIN
    ...
    RTOS.LockHeap();
      ...
      res := AllocTraced(info.nBytes, info.alignment, pool);
      IF (res = NIL) THEN  RTOS.UnlockHeap(); RETURN NIL;  END;
...
PROCEDURE GetArrayInfo (def: RT0.TypeDefn;  READONLY s: Shape;
  ...
  BEGIN
    ...
    IF (def.typecode = 0)
      OR (def.traced = ORD (RK.Untraced) AND traced)
      OR (def.traced # ORD (RK.Untraced) AND NOT traced)
