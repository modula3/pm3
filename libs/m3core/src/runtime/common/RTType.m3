...
PROCEDURE IsSubtype (a, b: Typecode): BOOLEAN =
  ...
  BEGIN
    ...
    END;
    CASE t.traced OF
    | ORD (RK.Untraced)    => RETURN (b = RT0.AddressTypecode);
    | ORD (RK.Traced)      => RETURN (b = RT0.RefanyTypecode);
    | ORD (RK.Transient)   => RETURN (b = RT0.TRefanyTypecode);
    ELSE
...
PROCEDURE IsTraced (tc: Typecode): BOOLEAN =
  VAR t := Get (tc);
  BEGIN
    RETURN (t.traced # ORD (RK.Untraced));
  END IsTraced;

PROCEDURE IsTransient (tc: Typecode): BOOLEAN =
  VAR t := Get (tc);
  BEGIN
    RETURN (t.traced = ORD (RK.Transient));
  END IsTransient;
...
PROCEDURE AssignBuiltinTypes () =
  BEGIN
    ...
    GenBuiltin (ADR (TREFANY_typecell),  "<*TRANSIENT*> REFANY");
    GenBuiltin (ADR (TROOT_typecell),    "<*TRANSIENT*> ROOT");
...
VAR
  TREFANY_typecell := RT0.Typecell {
    typecode      := RT0.TRefanyTypecode,
    selfID        := TREFANY_uid,
    fp            := RT0.Fingerprint { 16_2d, 16_a9, 16_83, 16_79,
                                       16_14, 16_1e, 16_67, 16_28},
    traced        := ORD (RK.Transient),
    kind          := ORD (TK.Ref),
    link_state    := 0,
    dataAlignment := BYTESIZE (ADDRESS),
    dataSize      := BYTESIZE (ADDRESS),
...
VAR
  TROOT_typecell := RT0.ObjectTypecell {
    common := RT0.Typecell {
      typecode      := RT0.TRootTypecode,
      selfID        := -1452026970,
      fp            := RT0.Fingerprint {16_09, 16_fb, 16_43, 16_7d,
                                        16_af, 16_28, 16_30, 16_d4},
      traced        := ORD (RK.Transient),
      kind          := ORD (TK.Obj),
      link_state    := 0,
      dataAlignment := BYTESIZE (ADDRESS),
