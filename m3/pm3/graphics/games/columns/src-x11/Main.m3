...
PROCEDURE AddArgT (VAR x: ArgList;  label: Xt.String;  text: TEXT) =
  BEGIN
    x.a := ADR (x.v[0]);
    WITH n = x.v[x.n] DO  n.name := label;  n.value := FlatTtoS (text)  END;
    INC (x.n);
  END AddArgT;
...
PROCEDURE CreateButton (label: TEXT;  handler: Xt.CallbackProc): Xt.Widget =
  ...
  BEGIN
    ...
    w := Xt.CreateManagedWidget (FlatTtoS (label), Xaw.commandWidgetClass,
                                 form, args.a, args.n);
...
PROCEDURE CreateLabel (text: TEXT): Xt.Widget =
  ...
  BEGIN
    ...
    w := Xt.CreateManagedWidget (FlatTtoS (text), Xaw.labelWidgetClass,
                                 form, args.a, args.n);
    lastChild := w;
    RETURN w;
  END CreateLabel;
...
PROCEDURE Init () =
  ...
  BEGIN
    ...
    AddArgI (args, XtN.input,       ORD (TRUE));
    box := Xt.CreateManagedWidget (FlatTtoS ("outerbox"), Xaw.panedWidgetClass,
    ...
    form := Xt.CreateManagedWidget (FlatTtoS ("form"), Xaw.boxWidgetClass, box,
                                      args.a, args.n);
    ...
    AddArgI (args, XtN.input,         ORD (TRUE));
    AddArgI (args, XtN.showGrip,      ORD (FALSE));
    playingArea := Xt.CreateManagedWidget (FlatTtoS ("playingArea"),
