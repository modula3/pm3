MODULE SchemeTest2 EXPORTS Main;
(**
  - Schema declaration
  - List buildup
  ...
  - Simulating a direct dependency with a dependency
    over 3 edges using dummy attributes        *)
...
PROCEDURE InitScheme () =
  ...
  BEGIN
    ...
    AGDeclareAttribute(
      ...
      IndexProperties.Normal, IntegerValue, Cardinality.OblUnique, FALSE,
      10, IntrAttr);
    AGSetEvaluationFunction(Scheme, "NODE", "IntrinsicNo", "FctOne");

    (** intrinsic key KeyNo : integer; *)
    AGDeclareAttribute(Scheme, "NODE", "KeyNo", AttributeKind.Intrinsic,
                       IndexProperties.Key, IntegerValue,
                       Cardinality.OblUnique, FALSE, 10, KeyAttr);

    (** derived DerivedNo : integer = [ self.<-Next-.DerivedNo | 0 ] + 1; *)
    AGDeclareAttribute(Scheme, "NODE", "DerivedNo", AttributeKind.Derived,
                       IndexProperties.Normal, IntegerValue,
                       Cardinality.OblUnique, FALSE, 10, DerAttr);

    (** derived index IndexNo = self.DerivedNo % 5; *)
    AGDeclareAttribute(Scheme, "NODE", "IndexNo", AttributeKind.Derived,
                       IndexProperties.Index, IntegerValue,
                       Cardinality.OblUnique, FALSE, 10, IndAttr);
    ...
    (**
      Dummy-Attribute :
        ...
        is replaced by 3 dummy attributes over 3 edges
    *)

    AGDeclareAttribute(
      Scheme, "NODE", "Dummy", AttributeKind.Dummy, IndexProperties.Normal,
      UnknownValue, Cardinality.OblUnique, TRUE, 0, DummyAttr);
    AGDeclareDependency(Scheme, "NODE", "DerivedNo", "Dummy", "NEXT",
                        DependencyKind.IncomingDependent);
    AGDeclareDependency(Scheme, "NODE", "Dummy", "Dummy", "NEXT",
                        DependencyKind.OutgoingDependent);
    AGDeclareDependency(Scheme, "NODE", "Dummy", "DerivedNo", "NEXT",
                        DependencyKind.IncomingDependent);
