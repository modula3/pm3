...
PROCEDURE rmCall (t: T; READONLY op: Operand; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    ...
    do_relocs := build_modrm(t, op, t.opcode[2], ins, indreg);
    writecode(t, ins);
...
PROCEDURE setccOp (t: T; READONLY op: Operand; cond: Cond;
  ...
  BEGIN
    ...
    END;
    do_relocs := build_modrm(t, op, t.opcode[0], ins, indreg);
    ins.escape := TRUE;
  ...
  BEGIN
    ...
    IF t.ftop_inmem THEN
      ...
      do_relocs := build_modrm(t, Operand {loc := OLoc.mem, mvar := t.ftop_mem},
                               t.opcode[fopcode[op].memop], mem, indreg);
      writecode(t, mem);
      IF do_relocs THEN 
...
PROCEDURE memFOp (t: T; op: FOp; mvar: MVar; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    ...
    do_relocs := build_modrm(t, Operand {loc := OLoc.mem, mvar := mvar},
                             t.opcode[fopcode[op].memop], ins, indreg);
...
PROCEDURE immOp (t: T; op: Op; READONLY dest: Operand; imm: INTEGER;
  ...
  BEGIN
    ...
    IF dest.loc = OLoc.register AND dest.reg = EAX
       AND ins.imsize = 4 AND NOT t.pic THEN		(* modif. PIC *)
      ins.opcode := opcode[op].Aimm32;
      writecode(t, ins);
    ELSE
      do_relocs := build_modrm(t, dest, t.opcode[opcode[op].immop], ins,
                               indreg);
...
PROCEDURE binOp (t: T; op: Op; READONLY dest, src: Operand;
  ...
  BEGIN
    ...
    IF dest.loc = OLoc.register THEN
      do_relocs_src := build_modrm(t, src, dest, ins, indreg);
    ...
    ELSE
      ...
      do_relocs_dest := build_modrm(t, dest, src, ins, indreg);
...
PROCEDURE tableOp (t: T; op: Op; READONLY dest, index: Operand;
  ...
  BEGIN
    ...
    IF (NOT fully_known) OR (ins.disp > 16_7f) OR (ins.disp < -16_80) THEN
      ...
      IF fully_known OR t.pic THEN INC (ins.modrm, 16_80); END;
    ELSE
      ins.dsize := 1;
...
PROCEDURE movImm (t: T; READONLY dest: Operand; imm: INTEGER;
  ...
  BEGIN
    IF dest.loc # OLoc.register THEN
      ...
      ins.opcode := 16_C6;
      get_op_size(dest.mvar.t, ins);
      do_relocs := build_modrm(t, dest, t.opcode[0], ins, indreg);
...
PROCEDURE pushOp (t: T; READONLY src: Operand; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    ...
    | OLoc.mem =>
        ...
        do_relocs := build_modrm(t, src, t.opcode[6], ins, indreg);
        ins.opcode := 16_FF;
        writecode(t, ins);
        IF do_relocs THEN
          log_global_var(t, src.mvar, -4);
...
PROCEDURE popOp (t: T; READONLY dest: Operand; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    ...
    | OLoc.mem =>
        ...
        do_relocs := build_modrm(t, dest, t.opcode[6], ins, indreg);
        ins.opcode := 16_FF;
        writecode(t, ins);
        IF do_relocs THEN
          log_global_var(t, dest.mvar, -4);
...
PROCEDURE decOp (t: T; READONLY op: Operand; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    ...
    ELSE
      ...
      do_relocs := build_modrm(t, op, t.opcode[1], ins, indreg);
      ins.opcode := 16_FF;
      writecode(t, ins);
      IF do_relocs THEN
        log_global_var(t, op.mvar, -4);
...
PROCEDURE unOp (t: T; op: Op; READONLY dest: Operand; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    ...
    do_relocs := build_modrm(t, dest, t.opcode[opcode[op].immop], ins, indreg);
...
PROCEDURE mulOp (t: T; READONLY src: Operand; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    ...
    do_relocs := build_modrm(t, src, t.opcode[4], ins, indreg);
...
PROCEDURE imulOp (t: T; READONLY dest, src: Operand; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    ...
    IF src.loc = OLoc.imm THEN
      do_relocs := build_modrm(t, t.reg[dest.reg], dest, ins, indreg);
      ins.opcode := 16_69;
      ins.imm := src.imm;
      ins.imsize := 4;
      writecode(t, ins);
    ELSE
      do_relocs := build_modrm(t, src, dest, ins, indreg);
      ins.escape := TRUE;
      ins.opcode := 16_AF;
      writecode(t, ins);
    END;
...
PROCEDURE divOp (t: T; READONLY divisor: Operand; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    ...
    do_relocs := build_modrm(t, divisor, t.opcode[6], ins, indreg);
...
PROCEDURE idivOp (t: T; READONLY divisor: Operand; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    <* ASSERT divisor.loc = OLoc.register OR (divisor.loc = OLoc.mem
              AND CG_Bytes[divisor.mvar.t] = 4) *>
    do_relocs := build_modrm(t, divisor, t.opcode[7], ins, indreg);
  ...
  BEGIN
    ...
    IF (NOT fully_known) OR (offset > 16_7f) OR (offset < -16_80) THEN
      ...
      IF NOT fully_known THEN
        IF t.pic THEN
          ins.modrm := 16_80 + reg.reg*8 + indreg.reg;
          IF indreg.reg # 3 THEN
            relocs_to_do := FALSE; (* indreg contains the relocated addr. *)
          END;
        ELSE
          ins.modrm := reg.reg*8 + 5;
        END;
      ELSE
        ins.modrm := 16_80 + reg.reg*8 + EBP;
      END;
    ELSE
      ins.disp := offset;
      ins.dsize := 1;
  ...
  BEGIN
    ...
    WITH cursText   = t.obj.cursor(Seg.Text),
      ...
      IF t.pic THEN
        ...
        ins.sib    := 16_80 + index.reg * 8 + indreg.reg; ins.sibpres := TRUE;
        writecode(t, ins);
	...
	END;
        ...
        ins.disp    := cursRodata;              ins.dsize   := 4;
        writecode(t, ins);
        (* Jump to abs address indexed by register 'index'*4 *)
      ...
      FOR i := 0 TO NUMBER(labels) - 1 DO
	    ...
	    IF t.pic THEN
              ...
              t.obj.relocate(t.datasym, cursData + i * 4, t.textsym, R_386_32);
	    ELSE
...
PROCEDURE fstack_loadtop (t: T; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    ...
    END;
    do_relocs := build_modrm(t, Operand {loc := OLoc.mem, mvar := t.ftop_mem},
                             t.opcode[fopcode[FOp.fLD].memop], ins, indreg);
    writecode(t, ins);
    IF do_relocs THEN
      log_global_var(t, t.ftop_mem, -4);
    END;
...
PROCEDURE fstack_pop_inreg (t: T; READONLY mvar: MVar; indreg:= BASE_FOR_PIC) =
  ...
  BEGIN
    ...
    END;
    do_relocs := build_modrm(t, Operand {loc := OLoc.mem, mvar:= mvar},
                             t.opcode[fopcode[FOp.fSTP].memop], ins, indreg);
    writecode(t, ins);
    IF do_relocs THEN
      log_global_var(t, mvar, -4);
    END;
...
PROCEDURE fstack_swap (t: T) =
  ...
  BEGIN
    ...
    Mn(t, "FLD ST, m80real");
    EVAL build_modrm(t, t.fstackspill[t.fspilltop-2], t.opcode[5], ins);
    ins.opcode := 16_DB;
    writecode(t, ins);

    Mn(t, "FLD ST, m80real");
    EVAL build_modrm(t, t.fstackspill[t.fspilltop-1], t.opcode[5], ins);
    ins.opcode := 16_DB;
    writecode(t, ins);
...
PROCEDURE get_temp (t: T) =
  ...
  BEGIN
    ...
    WITH spill = t.fstackspill[t.fspilltop] DO
      ...
      EVAL build_modrm(t, spill, t.opcode[7], ins);
      ins.opcode := 16_DB;
      writecode(t, ins);
    END;
    INC(t.fspilltop);
...
PROCEDURE retrieve_temp (t: T) =
  ...
  BEGIN
    ...
    WITH spill = t.fstackspill[t.fspilltop] DO
      Mn(t, "FLD ST, m80real");
      EVAL build_modrm(t, spill, t.opcode[5], ins);
      ins.opcode := 16_DB;
      writecode(t, ins);
    END;
  END retrieve_temp;
...
PROCEDURE log_global_var (t: T; mvar: MVar; reltocurs: INTEGER) =
  ...
  BEGIN
    ...
    ELSE
      IF t.pic THEN
        IF mvar.var.imported THEN
          t.obj.patch(Seg.Text, patch_loc, 0, 4);
          t.obj.relocate(t.textsym, patch_loc, mvar.var.symbol, R_386_GOT32);
        ELSE
          t.obj.patch(Seg.Text, patch_loc, mvar.o + mvar.var.offset, 4);
          t.obj.relocate(t.textsym, patch_loc, mvar.var.symbol, R_386_GOTOFF);
        END;
      ELSE
        t.obj.patch(Seg.Text, patch_loc, mvar.o + mvar.var.offset, 4);
        t.obj.relocate(t.textsym, patch_loc, mvar.var.symbol, R_386_32);
      END;
    END
  END log_global_var;
...
PROCEDURE tidy_internals (t: T) =
  ...
  BEGIN
    ...
    IF internal_size # 0 THEN
      intvar := init_intvar(t, internal_size, fl_used);
...
PROCEDURE init_intvar (t: T; size: ByteSize; f_lit: FLiteral):
            x86Var =
  VAR intvar: x86Var;
      tint: Target.Int;
  BEGIN
