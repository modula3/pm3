...
PROCEDURE SetUp (t: T;  pkg, to_pkg, build_dir: TEXT)
  ...
  BEGIN
    ...
    Quake.Define (t, "PACKAGE", Pathname.Last(pkg));
...
CONST
  Builtins = ARRAY OF Builtin {
    ...
    Builtin {"_install_file",                 Do_InstallFile,    4, FALSE},
...
PROCEDURE DoInsDerivedLnk (m: QMachine.T;  <*UNUSED*> n_args: INTEGER)
  RAISES {Quake.Error} =
  VAR t := Self (m);  from, to: TEXT;
  BEGIN
    to   := PopText (t);
    from := PopText (t);
    InstallDerivedLink (t, from, to);
  END DoInsDerivedLnk;

PROCEDURE DoInsLnkToDerived (m: QMachine.T;  <*UNUSED*> n_args: INTEGER)
  RAISES {Quake.Error} =
  VAR t := Self (m);  src, dest: TEXT;
  BEGIN
    dest := PopText (t);
    src  := PopText (t);
    InstallLinkToDerived (t, src, dest);
  END DoInsLnkToDerived;
...
PROCEDURE Do_InstallFile (m: QMachine.T;  <*UNUSED*> n_args: INTEGER)
  RAISES {Quake.Error} =
  VAR
    t := Self(m);
    derived := PopBool (t);
    ...
    src := PopText (t);
  BEGIN
    InstallFile (t, src, dest, mode, derived);
  END Do_InstallFile;
...
PROCEDURE BeforeDoM3Hooks (m: QMachine.T)
  RAISES {Quake.Error, Thread.Alerted} =
  VAR
    t := Self (m);
    b := t.lookup (M3ID.Add ("before_do_m3_hooks"));
  BEGIN
    IF (b # NIL) THEN
      t.start_call (b.value);
      t.call_proc (0, FALSE);
