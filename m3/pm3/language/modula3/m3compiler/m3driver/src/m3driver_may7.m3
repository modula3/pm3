...
VAR
  ...
  ast_cache := NEW (IntRefTbl.Default).init ();
...
PROCEDURE EvaluateCacheSize() =
  ...
  VAR n : INTEGER:= 0;
      ...
      ref  : REFANY;
  BEGIN
    IF NOT cache_mode THEN  RETURN;  END;

    file_name := "cache" & Fmt.Int(sequence);
    INC(sequence);
    wr := FileWr.Open(file_name);

    WHILE it.next(name, ref) DO
(************** Could be Msg.Verbose in the future *******************)
Wr.PutText(wr, "\n\rThis interface is in the cache : " & M3ID.ToText(name) );
      INC(n);
    END;
Wr.PutText(wr, "\n\rThere are " & Fmt.Int(n) & " interfaces in the cache\n\r\n\r");
Wr.Close(wr);
...
(*  FOR i := 1 TO Params.Count - 1 DO
    ...
    IF (dump_config) THEN
      ...
      M3Timers.Stop ();
      IF cache_mode THEN
        Msg.Out("\nCan not be in cache_mode with this option...\n");
      END;
      CleanUp();
      Process.Exit (0);
  ...
  BEGIN
    ...
    | 'c' => IF (arg_len = 2) THEN
             ...
             ELSIF Text.Equal (arg, "-console") THEN
               gui := FALSE;  ok := TRUE;
	     ELSIF Text.Equal (arg, "-cache_mode") THEN
               cache_mode:= TRUE;  ok := TRUE;
             END;
