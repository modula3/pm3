...
(* Last modified on Mon Jan 30 15:06:42 PST 1995 by kalsow                   *)
(*      modified on Tue Nov 10 11:01:53 PST 1992 by muller                   *)
(*      modified on Tue Jun 23 10:51:31 PDT 1992 by schilit@xerox.com        *)
...
REVEAL
  T = BRANDED REF RECORD
      ...
      width:       REAL;   (* Current nominal line width *)
      indent:      REAL;   (* Current offset for breaks *)
...
VAR (*CONST*)
  ...
  reals                     := ARRAY [1 .. MaxReals] OF Real{NIL, ..};
  chars: ARRAY CHAR OF Char;
  bools: ARRAY BOOLEAN OF Bool;
  breakTypes: ARRAY BreakType OF REF BreakType;

TYPE
  OpProc = PROCEDURE (t: T; m: Mode; VAR p: Position; maxL: CARDINAL; args: CARDINAL): BOOLEAN
              RAISES {FBE.Failed, Thread.Alerted};
...
PROCEDURE New(fbe: FBE.T): T RAISES {} =
  BEGIN
    WITH t = NEW(T) DO
      t.fbe := fbe;
      t.width := t.fbe.PageWidth();
      t.indent := 0.0;
...
PROCEDURE UnderlyingFBE(t: T): FBE.T RAISES {} =
  BEGIN
    RETURN t.fbe;
  END UnderlyingFBE;

PROCEDURE Flush(t: T) RAISES {FBE.Failed} =
  <*FATAL Thread.Alerted*>
  BEGIN
...
PROCEDURE Close(t: T) RAISES {FBE.Failed} =
  BEGIN
    Flush(t);
...
PROCEDURE Group(t: T) RAISES {FBE.Failed} =
  BEGIN
    IF t.numChars > 0 THEN AddChars(t) END;
    AddRef(t, GroupOp);
  END Group;

PROCEDURE Begin(t: T;  offset := 0.0;  width := FBE.MaxWidth)
  RAISES {FBE.Failed} =
  BEGIN
    IF t.numChars > 0 THEN AddChars(t) END;
    AddRef(t, BeginOp);
    AddRef(t, NewReal(offset));
    AddRef(t, NewReal(width));
  END Begin;

PROCEDURE End(t: T) RAISES {FBE.Failed} =
  BEGIN
    IF t.numChars > 0 THEN AddChars(t) END;
    AddRef(t, EndOp);
  END End;

PROCEDURE SetFont (t: T; font: FBE.Font) RAISES {FBE.Failed} =
    BEGIN
     IF t.numChars > 0 THEN AddChars(t) END;
     AddRef(t, FontOp);
     AddRef(t, font);
...
PROCEDURE PutChar(t: T;  c: CHAR) RAISES {FBE.Failed} =
  BEGIN
    IF c = '\n' THEN
...
PROCEDURE PutText(t: T;  text: Text.T;  raw := FALSE) RAISES {FBE.Failed} =
  BEGIN
    IF raw THEN
      IF t.numChars > 0 THEN AddChars(t) END;
      AddRef(t, text);
...
PROCEDURE Break (t: T;  offset := 0.0;  type := BreakType.OptimalBreak;  freshLine := TRUE)
  RAISES {FBE.Failed} =
  BEGIN
    IF t.numChars > 0 THEN AddChars (t) END;
    AddRef(t, BreakOp);
...
PROCEDURE PartialBreak(t: T;  offset := 0.0;  freshLine := TRUE)
  RAISES {FBE.Failed} =
  BEGIN
    IF t.numChars > 0 THEN AddChars(t) END;
...
PROCEDURE UnitedBreak(t: T;  offset := 0.0;  freshLine := TRUE)
  RAISES {FBE.Failed} =
  BEGIN
    IF t.numChars > 0 THEN AddChars(t) END;
...
PROCEDURE NewLine(t: T;  offset := 0.0;  freshLine := TRUE)
  RAISES {FBE.Failed} =
  BEGIN
    IF t.numChars > 0 THEN AddChars(t) END;
    AddRef(t, NewLineOp);
...
PROCEDURE NoAlign(t: T) RAISES {FBE.Failed} =
  BEGIN
    IF t.numChars > 0 THEN AddChars(t) END;
    AddRef(t, NoAlignOp);
  END NoAlign;

PROCEDURE Col (t: T; column: REAL; relative := FALSE; space := 0.0)
  RAISES {FBE.Failed} =
  BEGIN
    IF t.numChars > 0 THEN AddChars(t) END;
    AddRef(t, ColOp);
    AddRef(t, NewReal(column));
    AddRef(t, bools[relative]);
    AddRef(t, NewReal(space));
  END Col;

PROCEDURE AddRef(t: T; ref: REFANY) RAISES {FBE.Failed, Thread.Alerted} =
  (*
...
PROCEDURE AddChars(t: T) RAISES {FBE.Failed} =
  BEGIN
...
PROCEDURE Allocate(
    ...
    this: Who;
    minSize: CARDINAL;
  ): CARDINAL RAISES {Thread.Alerted, FBE.Failed} =
  (*
...
PROCEDURE WaitUntilEmpty(t: T;  index: CARDINAL) RAISES {FBE.Failed} =
  (*
...
(*
  These procedures are used to pass FBE.Failed codes from 
...
PROCEDURE CheckForFailure(t: T;  who: Who) RAISES {FBE.Failed} =
  BEGIN
    IF (who = Who.Producer) AND (t.failure) THEN
      RAISE FBE.Failed(t.failureCode);
    END;
  END CheckForFailure;
...
<*INLINE*> 
PROCEDURE Probe(t: T; i: CARDINAL) RAISES {Thread.Alerted, FBE.Failed} =
  (*
...
<*INLINE*>
PROCEDURE GetR (t: T; i: CARDINAL): REAL RAISES {} =
  BEGIN
    RETURN NARROW(t.buffer[i MOD t.bufSize], Real)^;
  END GetR;
...
PROCEDURE PeekOp(t: T; i: CARDINAL): Op
  RAISES {Thread.Alerted, FBE.Failed} =
  BEGIN
    WITH r = Peek(t, i) DO
      TYPECASE r OF
      | NULL   => RETURN TextOp;
...
TYPE
  Position = RECORD
      ...
      c: REAL; (* current column, 0-based *)
      b: REAL; (* trailing blanks after .c *)
      font: FBE.Font := FBE.DefaultFont; (* current font to use *) 
      spaceWidth : REAL; (* cached value *)
...
PROCEDURE PrintTop(self: ConsumerThreadClosure): REFANY RAISES {} =
  ...
  BEGIN
    ...
    | FBE.Failed(failureCode) => SetFailure(self.t, failureCode);
    END;
    RETURN NIL;
  END PrintTop;
...
PROCEDURE Print(
    ...
    VAR pos: Position; 
    maxL: CARDINAL
  ): BOOLEAN RAISES {Thread.Alerted, FBE.Failed} =
  (*
...
PROCEDURE EnterBegin(
  ...
  BEGIN
    ...
    IF width < FBE.MaxWidth THEN t.width := width END;
    IF offset < FBE.MaxWidth THEN
      t.indent := MAX (0.0, pos.c + pos.b + offset);
    END;
...
PROCEDURE PrintFont (t   : T;
          <*UNUSED*> mode: Mode;
                 VAR pos : Position;
          <*UNUSED*> maxL: CARDINAL;
                     args: CARDINAL  ): BOOLEAN =
  BEGIN
    pos.font := Get(t, args);
    pos.spaceWidth := t.fbe.CharWidth(' ', pos.font);
    RETURN FALSE;
  END PrintFont;
...
PROCEDURE DoPrintText(
  ...
  BEGIN
    IF pos.b > 0.0 THEN DoTrailingBlanks(t, mode, pos) END;
    IF mode = Mode.Writing THEN t.fbe.PutText(text,pos.font) END;
    pos.c := pos.c + t.fbe.TextWidth(text, pos.font);
...
PROCEDURE DoPrintChar(
  ...
  BEGIN
    IF ch^ = '\n' THEN
      <* ASSERT FALSE *>
      (* RETURN DoNewLine(t, mode, pos, maxL, -FBE.MaxWidth); *)
    ELSIF ch^ = ' ' THEN
      pos.b := pos.b + pos.spaceWidth;
      RETURN FALSE;
    ELSE
      IF pos.b > 0.0 THEN DoTrailingBlanks(t, mode, pos) END;
      IF mode = Mode.Writing THEN t.fbe.PutChar(ch^,pos.font) END;
      pos.c := pos.c + t.fbe.CharWidth(ch^,pos.font);
      RETURN (mode = Mode.Thinking) AND (pos.c > t.width);
    END;
  END DoPrintChar;
...
PROCEDURE PrintCol (
  ...
  VAR column := GetR(t, args);
  BEGIN
    WITH
      relative = GetB(t, args + 1),
      space = GetR(t, args + 2)
    DO
      IF relative THEN column := column + t.indent; END;
...
PROCEDURE PrintNewLine(
  ...
  VAR offset: REAL;  freshLine: BOOLEAN;
  BEGIN
    offset := GetR (t, args);
    freshLine := GetB (t, args + 1);
    RETURN DoLine(t, mode, pos, maxL, offset, freshLine);
...
PROCEDURE PrintUnitedBreak(
  ...
  BEGIN
    ...
    LOOP
      ...
      refany := Peek(t, pos.i);
      offset := NARROW (Peek(t, pos.i + 1), Real)^;
      freshLine := NARROW (Peek(t, pos.i + 2), Bool)^;
      pos.i := pos.i + 3;
    END;
  END PrintUnitedBreak;

TYPE Widths = REF ARRAY OF REAL;
...
PROCEDURE PrintAlign(
  ...
  BEGIN
    ...
    offset := GetR (t, args + 2);
    alignPred := Get  (t, args + 3);
    TRY
      oneLine := tryOneLine AND (pos.l = 0);
      EnterBegin(t, mode, pos, maxL, 0.0, t.width, beginState);
      ...
      LOOP (* While another run of rows exists *)
        ...
        LOOP (* While there are more rows in the run *)
          ...
          IF op = NoAlignOp THEN
            ...
            EXIT;
          END;
          IF (pos1.i > start) AND DoNewLine(t, Mode.Thinking, pos1, maxL, 0.0) THEN
            pos := pos1;
            RETURN TRUE;
          END;
        ...
        LOOP (* While there are more rows in the run *)
          ...
          IF Peek(t, pos.i) = NoAlignOp THEN
            IF Print(t, mode, pos, maxL) THEN RETURN TRUE END;
          ELSIF ((pos.i > start) AND DoNewLine(t, mode, pos, maxL, 0.0))
...
PROCEDURE NewReal (f: REAL): Real =
  VAR
    nextSlot        := 0;
    fl:      Real;
  BEGIN
    FOR i := 1 TO MaxReals DO
      IF reals[i] = NIL THEN nextSlot := i; EXIT; END;
      IF reals[i]^ = f THEN RETURN reals[i] END;
    END;
    fl := NEW(Real);
    fl^ := f;
    IF nextSlot # 0 THEN reals[nextSlot] := fl; END;
    RETURN fl;
  END NewReal;
...
BEGIN
  ...
  FontOp         := DefineOp(PrintFont,         1, 9);
  TextOp         := DefineOp(PrintText,         1, 8);
