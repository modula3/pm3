...
(* Copy the FSM. Pickle is a bit overweight for this but is a clean and
   simple way to obtain a deep copy. *)

PROCEDURE Copy(READONLY m: T; VAR result: T) =
  VAR
    pkl := TextWr.New();
  BEGIN
    Pickle.Write(pkl,m); <*NOWARN*>
    result := Pickle.Read(TextRd.New(TextWr.ToText(pkl))); <*NOWARN*>
  END Copy;
