...
FROM M3x86Rep IMPORT Operand, OLoc, NRegs, MVar, x86Var, x86Proc, Regno;
...
TYPE Public = OBJECT
      ...
      METHODS
        ...
        set_symbol(textsym, datasym: INTEGER);
        intCall (lab: Label);
        picCall (symbol: INTEGER); 
        relCall (rel: INTEGER);
        absCall (p: x86Proc);
        rmCall (READONLY op: Operand; indreg:= BASE_FOR_PIC);
        cleanretOp (psize: INTEGER);
        brOp (br: Cond; l: Label);
        setccOp (READONLY op: Operand; cond: Cond; indreg:= BASE_FOR_PIC);
        noargOp (op: Op);
        noargFOp (op: FOp);
        immFOp (op: FOp; im: FIm);
        binFOp (op: FOp; st: INTEGER; indreg:= BASE_FOR_PIC);
        memFOp (op: FOp; mvar: MVar; indreg:= BASE_FOR_PIC);
	fstack_loadtop (indreg:= BASE_FOR_PIC);
        assert_fstack (count: INTEGER);
        f_exitproc ();
        f_pushnew ();
        fstack_push (READONLY mvar: MVar);
        fstack_pop_inmem (READONLY mvar: MVar): BOOLEAN;
        fstack_pop_inreg (READONLY mvar: MVar; indreg:= BASE_FOR_PIC);
        fstack_swap ();
        fstack_discard ();
	fstack_check(depth: INTEGER; place: TEXT);
        ...
        tableOp (op: Op; READONLY dest, index: Operand; scale: INTEGER;
                 table: MVar);
        swapOp (READONLY dest, src: Operand; indreg:= BASE_FOR_PIC);
        movOp (READONLY dest, src: Operand; indreg:= BASE_FOR_PIC);
