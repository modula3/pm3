...
ifeq ($(patsubst %-m68k-hp-hpux9,m68k-hp-hpux9,$(host)),m68k-hp-hpux9)
...
ifeq ($(host),hppa1.1-hp-hpux9)
CC := cc -Wp,-H256000
endif

ifeq ($(host),m68k-hp-hpux9)
CC := cc -Wp,-P,-H256000
endif
...
configargs := $(configargs) --with-stabs
gconfigargs := $(gconfigargs) --enable-shared
endif

# And for the Alpha, since it uses the same tools.
...
# don't do --enable-gdbtk because we have a hosed system
ifeq ($(target),i386-ncr-sysv4.3)
configargs := --with-stabs
gconfigargs := $(gconfigargs) --enable-shared
endif

# Enable shared libraries on various other targets.
ifeq ($(patsubst %-linux,linux,$(target)),linux)
gconfigargs := $(gconfigargs) --enable-shared
endif
ifeq ($(patsubst sparc-sun-solaris2%,solaris,$(target)),solaris)
gconfigargs := $(gconfigargs) --enable-shared
endif
ifeq ($(target),hppa1.1-hp-hpux9)
gconfigargs := $(gconfigargs) --enable-shared
endif
ifeq ($(patsubst %-ibm-aix%,aix,$(target)),aix)
gconfigargs := $(gconfigargs) --enable-shared
endif

#We don't yet support building SunOS shared libraries with GNU ld.
#ifeq ($(patsubst sparc-sun-sunos4.1%,sun4,$(target)),sun4)
#gconfigargs := $(gconfigargs) --enable-shared
#endif
...
ifeq ($(patsubst %-a29k-vxworks5.1,a29k-vxworks5.1,$(target)),a29k-vxworks5.1)
configargs := $(configargs) \
	--with-headers=/s1/cygnus/dejagnu/$(target)/include
endif
...
$(host)-stamp-stage2-configured:
	...
	$(SET_CYGNUS_PATH) cd $(WORKING_DIR) ; \
	  $(configenv) $(GNUC) $(TIME) $(CONFIG_SHELL) ../$(TREE)/configure $(config) -v --srcdir=../$(TREE) $(prefixes) $(configargs) $(gconfigargs)
	touch $@
...
ifeq (i386-ncr-sysv4.3,$(holesys))
SET_NATIVE_HOLES := SHELL=$(SHELL) ; PATH=`pwd`/$(HOLESDIR):/usr/ccs/bin ; export PATH ; export SHELL ;
HOLE_DIRS := /usr/ccs/bin
endif
