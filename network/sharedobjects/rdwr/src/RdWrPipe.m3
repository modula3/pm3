 ...
 *
 * Revision 1.2  2003/01/27 17:07:14  hosking
 * Transients got lost somewhere.
 *
...
TYPE
  ...
  (* our reader and writer will share a buffer and an instance of the
     SharedData RECORD *)
  RdT = Rd.T OBJECT
          share: <*TRANSIENT*> REF SharedData;
        OVERRIDES
          seek   := RdSeek;
          length := RdLength;
          close  := RdClose;
...
PROCEDURE New (VAR rd       : Rd.T;
  ...
  VAR
    info := NEW(<*TRANSIENT*> REF SharedData, mu := NEW(Thread.Mutex),
                cv := NEW(Thread.Condition), size := buff_size);
    shared_buff := NEW(<*TRANSIENT*> REF ARRAY OF CHAR, buff_size);
  BEGIN
