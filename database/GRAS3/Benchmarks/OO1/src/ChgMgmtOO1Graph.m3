...
IMPORT Access, ChgMgmtGraph, ErrorSupport, PageFile, CardSet, Node,
       NodeSet, PersistentGraph, ChgMgmtGraphPool;
IMPORT OO1Graph;
CONST
  PartLabel       = 1;
  ConnectionLabel = 2;
...
PROCEDURE Open (gr            : T;
  ...
  <* FATAL  ChgMgmtGraphPool.NotInTransaction *>
  BEGIN
    TRY
      gr.pool := NEW(ChgMgmtGraphPool.T).open(name, access, new);
      gr.graph := NEW(ChgMgmtGraph.T).open(
                    gr.pool, name, ChgMgmtGraph.AccessMode.Inherit, new,
                    FALSE (*errorChecks*), local);
    ...
    | ChgMgmtGraph.InternalError (info) => HandleInternalError(info);
    | ChgMgmtGraphPool.InternalError (info) => HandleInternalError(info);
    | ChgMgmtGraph.InUse => RAISE OO1Graph.Failure("Graph in use!");
    | ChgMgmtGraph.NotExistent, ChgMgmtGraphPool.NotExistent =>
...
PROCEDURE Close (gr: T) =
  BEGIN
    TRY
      ...
      gr.pool.close();
    EXCEPT
    | ChgMgmtGraph.InternalError => (* I don't care *)
    | ChgMgmtGraphPool.InternalError => (* I don't care *)
    END;
  END Close;
...
PROCEDURE CreatePart (gr: T; n: CARDINAL): CARDINAL
  ...
  BEGIN
    ...
    EXCEPT
      Access.Locked => RAISE OO1Graph.Failure("Access locked");
    | ChgMgmtGraph.InternalError (info) => HandleInternalError(info);
    | ChgMgmtGraph.LogError (info) => HandleInternalError(info);
